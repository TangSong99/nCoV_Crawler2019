...
public class Parse {
     ...
     * @author CTX
     * @date 2020/2/5
     * @param  rymorInformation
     * @return
     */
    public static List<RymorList> parseRymorInformation(String rymorInformation){
        List<RymorList> list = new ArrayList<>();
        JSONArray jsonArray = JSON.parseArray(rymorInformation);
        for(Object jsonObj : jsonArray){
            RymorList rymorList = JSON.toJavaObject((JSON)jsonObj,RymorList.class);
            list.add(rymorList);
            logger.info(rymorList);
        }
        return list;
    }
}
